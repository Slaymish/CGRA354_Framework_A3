...
void Application::render() {
  ...
  // ensure we draw to the entire window minus the ImGui sidebar
  glViewport(700, 0, width - 700,
             height); // Adjust viewport to the right of the ImGui menu
  ...
  float aspect = float(width - 700) /
                 height; // update aspect ratio to consider sidebar width
  glm::mat4 proj = glm::perspective(1.0f, aspect, 1.0f, 2000.0f);
  glm::mat4 view(1);
...
void Application::renderGUI() {
  ...
  // Set the window position and size to lock it to the left-hand side
  ...
  if (ImGui::TreeNode("Camera")) {
    ...
    ImGui::SliderFloat("Pitch", &m_pitch, -glm::pi<float>() / 2,
                       glm::pi<float>() / 2);
    ImGui::SliderFloat("Yaw", &m_yaw, -glm::pi<float>(), glm::pi<float>());
    ImGui::SliderFloat("Distance", &m_distance, 0, m_max_distance, "%.2f",
                       2.0f);
...
void Application::cursorPosCallback(double xpos, double ypos) {
  if (m_leftMouseDown) {
    ...
    m_pitch +=
        glm::acos(glm::clamp((m_mousePosition.y - viewport_hsize.y) /
                                 viewport_hsize.y,
                             -1.0f, 1.0f)) -
        glm::acos(glm::clamp(
            (float(ypos) - viewport_hsize.y) / viewport_hsize.y, -1.0f, 1.0f));
    m_pitch = glm::clamp(m_pitch, -glm::pi<float>() / 2, glm::pi<float>() / 2);

    // wrap the yaw to [-pi, pi]
    m_yaw +=
        glm::acos(glm::clamp((m_mousePosition.x - viewport_hsize.x) /
                                 viewport_hsize.x,
                             -1.0f, 1.0f)) -
        glm::acos(glm::clamp(
            (float(xpos) - viewport_hsize.x) / viewport_hsize.x, -1.0f, 1.0f));
...
void Application::scrollCallback(double xoffset, double yoffset) {
  ...
  // logrithmic zoom
  float d = m_max_distance * pow(pow(m_distance / m_max_distance, 0.5f) -
                                     (float(yoffset) / 50.0f),
                                 2.0f);
  if (d == d && !isinf(d))
...
void Application::charCallback(unsigned int c) { (void)c; }